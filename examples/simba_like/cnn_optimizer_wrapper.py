...
class CNNOptimizer:
    ...
    def uneven_tuner(self, fnl_res):
        ...
        pr_bfr_tls, pr_bfr_unevn = fx_unevn_L0(pr_bfr_tls, sprt_strct, memrs_flat)
    ...
    def solve(self, threads=1, optimize_uneven_mapping=True):
        memrs = self.memrs
        ...
        ret, costs = tls_sptl_tmprl(
            ...
            mem_size=memrs["reg"], # combined with the by-pass list means capacity is only 1 weight element
        ...
        fnl_res = tls_tmprl_alpha_beta_mlt_thrd(
            tiles=L2_cndds,
            prob=self.problem_generic,
            mem_size=memrs["L2"],
            access_energies=access_e,
