...
class CNNOptimizer():
    def __init__(self, problem, accelerator):
        ...
        self.dw_problem_generic = GenericProblem(self.dw_bounds, self.dw_tens_desc)
    ...
    def solve(self, access_energies=None, EDP_flag=False, threads=1, sptl_cnstrnts=None):
        sizes = [self.accelerator.get_memory_data_capacity(level) for level in range(self.levels)]
        ...
        if self.accelerator.unified_L1:
            access_e.append([access_energies["L1"], access_energies["L1"], access_energies["L1"]])
        else:
            access_e.append([access_energies["L1"][2], access_energies["L1"][0], access_energies["L1"][1]])

        access_e.append([access_energies["L2"] + access_energies["communication"],
                         access_energies["L2"] + access_energies["communication"],
                         access_energies["L2"] + access_energies["communication"]])

        NoC = (self.accelerator.memory_hierarchy[1].read_bandwidth, self.accelerator.memory_hierarchy[1].write_bandwidth)
        ...
        # ******************* L2 Optimization *******************
        access_e.append([access_energies["L3"], access_energies["L3"], access_energies["L3"]])
